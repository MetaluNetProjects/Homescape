...
#if ((defined PD_MAJOR_VERSION && defined PD_MINOR_VERSION) && (PD_MAJOR_VERSION > 0 || PD_MINOR_VERSION > 46))
static void init_loadbang(t_init *x, t_floatarg action)
{
  if(action == 0 && !sys_noloadbang)// if action is LB_LOAD, not LB_CLOSE or LB_INIT
    init_bang(x);
}
#else
...
#if ((defined PD_MAJOR_VERSION && defined PD_MINOR_VERSION) && (PD_MAJOR_VERSION > 0 || PD_MINOR_VERSION > 46))
  class_addmethod(init_class, (t_method)init_loadbang, gensym("loadbang"), A_FLOAT, 0);
#else
