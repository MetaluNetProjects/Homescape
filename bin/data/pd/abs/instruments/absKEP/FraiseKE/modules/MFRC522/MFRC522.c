 ...
 */
/*MFRC522_MFRC522(	byte chipSelectPin,		///< Arduino pin connected to MFRC522's SPI slave select input (Pin 24, NSS, active low)
 ...
 */
void MFRC522_PCD_WriteRegister(	byte reg,		///< The register to write to. One of the PCD_Register enums.
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
byte MFRC522_PCD_ReadRegister(	byte reg	///< The register to read from. One of the PCD_Register enums.
								) {
	byte value;
	digitalWrite(_chipSelectPin, LOW);			// Select slave
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void MFRC522_PCD_ReadRegister(	byte reg,		///< The register to read from. One of the PCD_Register enums.
 ...
 */
void MFRC522_PCD_SetRegisterBitMask(	byte reg,	///< The register to update. One of the PCD_Register enums.
 ...
 */
void MFRC522_PCD_ClearRegisterBitMask(	byte reg,	///< The register to update. One of the PCD_Register enums.
 ...
 */
byte MFRC522_PCD_CalculateCRC(	byte *data,		///< In: Pointer to the data to transfer to the FIFO for CRC calculation.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte MFRC522_PCD_TransceiveData(	byte *sendData,		///< Pointer to the data to transfer to the FIFO.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte MFRC522_PCD_CommunicateWithPICC(	byte command,		///< The command to execute. One of the PCD_Command enums.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte MFRC522_PICC_RequestA(byte *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte MFRC522_PICC_WakeupA(	byte *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
							byte *bufferSize	///< Buffer size, at least two bytes. Also number of bytes returned if STATUS_OK.
							) {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte MFRC522_MIFARE_TwoStepHelper(	byte command,	///< The command to use
 ...
 */
const __FlashStringHelper *MFRC522_GetStatusCodeName(byte code	///< One of the StatusCode enums.
										) {
	switch (code) {
 ...
 * @return PICC_Type
 */
byte MFRC522_PICC_GetType(byte sak		///< The SAK byte returned from PICC_Select().
							) {
 ...
 * @return const __FlashStringHelper *
 */
const __FlashStringHelper *MFRC522_PICC_GetTypeName(byte piccType	///< One of the PICC_Type enums.
										) {
	switch (piccType) {
 ...
 */
void MFRC522_MIFARE_SetAccessBits(	byte *accessBitBuffer,	///< Pointer to byte 6, 7 and 8 in the sector trailer. Bytes [0..2] will be set.
...
bool MFRC522_MIFARE_SetUid(byte *newUid, byte uidSize, bool logErrors) {
	...
	byte status = PCD_Authenticate(MFRC522_PICC_CMD_MF_AUTH_KEY_A, (byte)1, &key, &uid);
	if (status != STATUS_OK) {
